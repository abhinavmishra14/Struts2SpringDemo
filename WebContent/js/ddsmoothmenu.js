...
var ddsmoothmenu = {
		...
		arrowimages: {
	down:[ 'downarrowclass', 'imgs/down.gif', 23], right:[ 'rightarrowclass', 'imgs/right.gif']
},
transition: {
	overtime: 300, outtime: 300
},
//duration of slide in/ out animation, in milliseconds
shadow: {
	enable: true, offsetx: 5, offsety: 5
},
//enable shadow?
showhidedelay: {
...
buildmenu: function ($, setting) {
	...
	$headers.each(function (i) {
		...
		if (smoothmenu.shadow.enable && ! smoothmenu.css3support) {
			...
			this._shadowoffset = {
					x:(this.istopheader? $subul.offset().left + smoothmenu.shadow.offsetx: this._dimensions.w), y:(this.istopheader? $subul.offset().top + smoothmenu.shadow.offsety: $curobj.position().top)
			};
			//store this shadow's offsets
			if (this.istopheader)
				$parentshadow = $(document.body); else {
					var $parentLi = $curobj.parents("li:eq(0)");
					$parentshadow = $parentLi.get(0).$shadow;
				}
			this.$shadow = $('<div class="ddshadow' +(this.istopheader? ' toplevelshadow': '') + '"></div>').prependTo($parentshadow).css({
				left: this._shadowoffset.x + 'px', top: this._shadowoffset.y + 'px'
		...
		$curobj.hover(
				function (e) {
					...
					$targetul.data('timers').showtimer = setTimeout(function () {
						header._offsets = {
								left: $curobj.offset().left, top: $curobj.offset().top
						};
						...
						if ($targetul.queue().length <= 1) {
							...
							$targetul.css({
								left: menuleft + "px", width: header._dimensions.subulw + 'px'
							}).animate({
							...
							if (smoothmenu.shadow.enable && ! smoothmenu.css3support) {
								...
								header.$shadow.css({
									overflow: '', width: header._dimensions.subulw + 'px', left: shadowleft + 'px', top: shadowtop + 'px'
								}).animate({
